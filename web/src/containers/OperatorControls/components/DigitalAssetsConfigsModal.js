...
const ConfigsModal = ({
	...
	const [sort, setSort] = useState(default_digital_assets_sort);
	...
	const handleConfirm = () => {
		onConfirm({ pinned_assets: items, default_digital_assets_sort: sort });
		onCloseDialog();
	};
					...
					<Fragment>
						...
						<div className="mb-4">
							<Group onChange={handleSort} value={sort}>
								{Object.entries(DIGITAL_ASSETS_SORT).map(([key, mode]) => (
									<Radio value={mode}>
										<span className="bold caps-first">{mode}</span>
										<span> </span>
										<span>({DIGITAL_ASSETS_SORT_EXP[key]})</span>
									</Radio>
								))}
							</Group>
						</div>
