...
class DonutChart extends Component {
	...
	renderSlice = (value, i, width, height) => {
		...
		if (!this.state.isData) {
			return (
				<g key={i}>
					<path d={arcj(value)} fill={colors.noData} />
					<text
			...
			const { symbol = '', stringId, balancePercentage } = data;
			return (
				<g key={i}>
					<path
						d={arcj(value)}
						className={classnames(`chart_${symbol}`, {
							slice_active: activeSlice,
						})}
						onMouseOver={() => this.handleHover(symbol)}
					...
					{activeSlice ? (
						<Fragment>
							...
							>
								{STRINGS.formatString(STRINGS[stringId], balancePercentage)}
							</text>
						</Fragment>
					) : null}
				</g>
